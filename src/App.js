...
class App extends Component {
  ...
  cariPenginapan = (event) => {
    this.setState({
      ...
      penginapane: this.state.semuaPenginapan.filter((inap) => new RegExp(event.target.value,'i').exec(inap.nama))
    })
  }
  ...
  render() {
    ...
    return (
      <div className="app">
        <div className="main">
          <div className="cari">
            <input type="text" placeholder="Cari...." value={this.state.cari} onChange={this.cariPenginapan} />
          </div>
          <div className="Penginapan">
            {this.state.penginapane.map(inapin => {
              return(
                <Penginapan key={inapin.id} inap={inapin} pilihPenginapan={this.pilihPenginapan} />
              )
            })}
          </div>
        </div>
